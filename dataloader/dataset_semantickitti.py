...
class voxel_dataset(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=255, return_test=True,
    ...
    def __getitem__(self, index):
        ...
        grid_ind = (np.floor((np.clip(xyz, min_bound, max_bound) - min_bound) / intervals)).astype(np.int32)
        ...
        processed_label = labels  # cvpr labels
        ...
        # print(f"ITEMS FROM VOXEL: {len(data_tuple)}")
        # print(f"ITEMS FROM VOXEL: {len(data_tuple)}")
        # print(f"voxel position: {voxel_position}")
        # print(f"label: {processed_label.shape}")
        # print(f"grid_ind: {grid_ind}")
        # print(f"labels: {labels.shape}")
        # print(f"return fea: {return_fea.shape}")
        # print(f"index: {index}")
        return data_tuple
...
'''def collate_fn_BEV(data):    
    ...
    voxel_label = np.stack(voxel_label).astype(np.int32)
    ...
    return xyz, torch.from_numpy(voxel_label), grid_ind_stack, point_label, xyz
'''
