...
def add_ann_adj_info_test(extra_tag):
    ...
    dataroot = './data/nuscenes_test/'
    nuscenes = NuScenes(nuscenes_version, dataroot)
    # for set in ['train', 'val']:
    # set = 'test'
    dataset = pickle.load(
        open('./data/nuscenes_test/%s_infos_%s.pkl' % (extra_tag, 'test'), 'rb'))
    for id in range(len(dataset['infos'])):
        if id % 10 == 0:
            print('%d/%d' % (id, len(dataset['infos'])))
        info = dataset['infos'][id]
        ...
        scene = nuscenes.get('scene', sample['scene_token'])
        dataset['infos'][id]['occ_path'] = \
            './data/nuscenes_test/gts/%s/%s'%(scene['name'], info['token'])
    with open('./data/nuscenes_test/%s_infos_%s.pkl' % (extra_tag, 'test'),
                'wb') as fid:
        pickle.dump(dataset, fid)
