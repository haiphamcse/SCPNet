...
class InceptionDWConv3d(nn.Module):
    ...
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125):
        ...
        self.dwconv_d = nn.Conv3d(gc, gc, kernel_size=(band_kernel_size, 1, 1), padding=(band_kernel_size//2, 0, 0), groups=gc)
...
class MetaNeXtStage(nn.Module):
    def __init__(
        ...
        self.grad_checkpointing = False
            ...
            #     self.downsample = nn.Sequential(
            #         norm_layer(in_chs),
            #         nn.Conv3d(in_chs, out_chs, kernel_size=1, stride=1),
            #     )
...
class MetaNeXt(nn.Module):
    ...
    def __init__(
        ...
        #     nn.Conv3d(numC_input, num_channels[0], kernel_size=3, stride=1, padding=1),
        #     norm_layer(num_channels[0])
        # )

        # self.stages = nn.Sequential()
        dp_rates = [x.tolist() for x in torch.linspace(0, drop_path_rate, sum(num_layer)).split(num_layer)]
        # stages = []
