...
class BEVDet(CenterPoint):
    ...
    def __init__(self, img_view_transformer, img_bev_encoder_backbone,
                 img_bev_encoder_neck, freeze_img_backbone=False, **kwargs):
    ...
    def image_encoder(self, img, stereo=False):
        context = torch.no_grad if self.freeze_img_backbone else torch.enable_grad
        with context():
            imgs = img
    ...
    def extract_feat(self, points, img, img_metas, **kwargs):
        ...
        img_feats, depth, frustum_coor, sem2d = self.extract_img_feat(img, img_metas, **kwargs)
        pts_feats = None
        return (img_feats, pts_feats, depth, frustum_coor, sem2d)
...
class BEVStereo4D(BEVDepth4D):
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,
        ...
        bev_feat, depth, frustum_coor, sem2d = self.img_view_transformer(
            [x, sensor2keyego, ego2global, intrin, post_rot, post_tran, bda,
             mlp_input], metas)
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]
        return bev_feat, depth, frustum_coor, sem2d, stereo_feat
    ...
    def extract_img_feat(self,
        ...
        sem2d_key_frame = None
        frustum_coor_key_frame = None
        feat_prev_iv = None
        for fid in range(self.num_frame-1, -1, -1):
            ...
            if key_frame or self.with_prev:
                ...
                inputs_curr = (img, sensor2keyego, ego2global, intrin,
                               ...
                               extra_ref_frame)
                if key_frame:
                    bev_feat, depth, frustum_coor, sem2d, feat_curr_iv = \
                        self.prepare_bev_feat(*inputs_curr)
                    depth_key_frame = depth
                    frustum_coor_key_frame = frustum_coor
                    sem2d_key_frame = sem2d
                else:
                    with torch.no_grad():
                        bev_feat, depth, frustum_coor, sem2d, feat_curr_iv = \
                            self.prepare_bev_feat(*inputs_curr)
        ...
        bev_feat = torch.cat(bev_feat_list, dim=1)
        x = self.bev_encoder(bev_feat)
        return x, depth_key_frame, frustum_coor_key_frame, sem2d_key_frame


    def extract_img_feat_tta(self,
                         img,
                         img_metas,
                         pred_prev=False,
                         sequential=False,
        ...
        bev_feat1 = torch.cat(bev_feat_list, dim=1)

        # NOTE TTA
        # breakpoint()
        bev_feat2 = torch.flip(bev_feat1, dims=(3,))
        # bev_feat3 = torch.flip(bev_feat1, dims=(4,))
        # bev_feat4 = torch.flip(bev_feat1, dims=(3,4,))

        bev_feat = torch.cat([bev_feat1, bev_feat2], dim=0)
        # breakpoint()

        x = self.bev_encoder(bev_feat)

        # x1, x2, x3, x4 = torch.split(x, 1, dim=0)

        # for i in range(len(x)):
        # x[0][1] = torch.flip(x[0][1], dims=(2,))
        # x[0][2] = torch.flip(x[0][2], dims=(3,))
        # x[0][3] = torch.flip(x[0][3], dims=(2,3,))
        # x[0] = torch.mean(x[0], dim=(0,), keepdim=True)

        # x2 = torch.flip(x2, dims=(3,))
        # x3 = torch.flip(x3, dims=(4,))
        # x4 = torch.flip(x2, dims=(3,4,))
