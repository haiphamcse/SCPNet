...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        return points # B x N x D x H x W x 3
    ...
    def pre_compute(self, input):
        if self.initial_flag:
            ...
            # self.voxel_frustum_coor = world2voxel(coor)
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            return bev_feat, depth, None
        else:
            coor = self.get_lidar_coor(*input[1:7], self.frustum)
            bev_feat = self.voxel_pooling_v2(
                coor, depth.view(B, N, self.D, H, W),
                tran_feat.view(B, N, self.out_channels, H, W))


            # coor_full = self.get_lidar_coor(*input[1:7], self.full_frustum)
            # return bev_feat, depth, coor_full
            return bev_feat, depth, coor
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def __init__(self, loss_depth_weight=3.0, depthnet_cfg=dict(), semnet_cfg=dict(), **kwargs):
        super(LSSViewTransformerBEVDepth, self).__init__(**kwargs)
        self.loss_depth_weight = loss_depth_weight
        self.depth_net = DepthNet(self.in_channels, self.in_channels,
                                  self.out_channels//2, self.D, **depthnet_cfg)

        self.sem_net = DepthNet(self.in_channels, self.in_channels, self.out_channels//2, 18, **semnet_cfg)

        self.depth_dice_loss = DiceLoss(reduction='none')
    ...
    def get_downsampled_gt_depth(self, gt_depths, downsample=True):
        """
        Input:
            gt_depths: [B, N, H, W]
        Output:
    ...
    def get_depth_loss(self, depth_labels, depth_preds):
        # need_downsample = (depth_labels.shape[2] != depth_preds.shape[2]) or (depth_labels.shape[3] != depth_preds.shape[3])
        # depth_labels = self.get_downsampled_gt_depth(depth_labels, downsample=need_downsample)
        ...
        depth_loss = depth_loss + self.depth_dice_loss(depth_preds, depth_labels).sum() / max(1.0, fg_mask.sum())
        return self.loss_depth_weight * depth_loss

    def forward(self, input, stereo_metas=None):
        (x_, rots, trans, intrins, post_rots, post_trans, bda,
         mlp_input) = input[:8]

        B, N, C, H, W = x_.shape
        x_ = x_.view(B * N, C, H, W)
        x = self.depth_net(x_, mlp_input, stereo_metas)
        sem = self.sem_net(x_, mlp_input, stereo_metas=None)
        
        sem2d = sem[:, :18, ...]
        depth_digit = x[:, :self.D, ...]

        tran_feat = torch.cat([x[:, self.D:self.D + self.out_channels//2, ...], sem[:, 18:18 + self.out_channels//2, ...]], dim=1)
        depth = depth_digit.softmax(dim=1)

        # breakpoint()
        bev_feat, depth, frustum_coor = self.view_transform(input, depth, tran_feat)
        return bev_feat, depth, frustum_coor, sem2d
...
class LSSViewTransformerBEVStereo(LSSViewTransformerBEVDepth):
    ...
    def __init__(self,  **kwargs):
        ...
        self.full_frustum = self.create_frustum(kwargs['grid_config']['depth'],
                                              kwargs['input_size'],
                                              downsample=1)
